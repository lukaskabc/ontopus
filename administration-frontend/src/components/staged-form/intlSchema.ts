...
const tOrNull = (i18n: i18n, key?: string) =>
  key && i18n.exists(key) ? i18n.t(key) : null
...
export default function intlSchema(
  schema: RJSFSchema,
  i18n: i18n,
  scope?: string
) {
  ...
  const newSchema = {
    ...
    ...pickBy({
      title:
        tOrNull(i18n, schema.title) ||
        tOrNull(i18n, `${scope}.title`) ||
        undefined,
      description:
        tOrNull(i18n, schema.description) ||
        tOrNull(i18n, `${scope}.description`) ||
        undefined,
      help:
        tOrNull(i18n, schema.help) ||
        tOrNull(i18n, `${scope}.help`) ||
        undefined,
    }),
  }

  if (schema.definitions) {
    newSchema.definitions = mapValues(
      schema.definitions,
      (typeSchema, typeName) =>
  ...
  if (schema.type === 'object') {
    newSchema.properties = mapValues(
      schema.properties,
      (propSchema, propName) =>
  ...
  if (
    schema.enum &&
    (!schema.enumNames || isEqual(schema.enum, schema.enumNames))
  ) {
