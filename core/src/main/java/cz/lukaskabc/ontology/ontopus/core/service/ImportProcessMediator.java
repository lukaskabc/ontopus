...
public class ImportProcessMediator {
    ...
    private void processAllResults(ImportProcessContext context, Map<String, FormResult> results) {
        while (context.hasUnprocessedService()) {
            ...
            context.handleResult(result); // TODO what if a single service will be in the pipeline twice??
            // and what if a proxy service like the result handling service is used???
            // what about generating ordered list of services
            // and prefixing the names with numbers or something to make them unique
        }
    }
